...
| PLUGIN-001 | in_progress | root/user | Run the first-party OpenWear desktop-local proof | New task created after desktop restart | Bundled CLI lists the project STDIO entry as enabled; create a new project task, confirm `/mcp`, and approve only `get_data_coverage` |
