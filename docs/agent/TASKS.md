...
| PLUGIN-001 | in_progress | root/user | Run the first-party OpenWear desktop-local workout flow | New task created after desktop restart | Bundled CLI lists the project STDIO entry as enabled; submit `/mcp`, verify coverage, then record/review a synthetic or real strength session with write approval |
